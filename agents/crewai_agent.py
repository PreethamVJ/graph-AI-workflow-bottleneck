...
def _env(*names: str, default: str | None = None) -> str | None:
    """First non-empty match across a list of env var names.

    Lets us rename OPENAI_* -> LLM_* without breaking anyone who already
    has an old .env. New names are tried first.
    """
    for n in names:
        v = os.getenv(n)
        if v:
...
def _resolve_llm_mode() -> str:
    """Decides which of three modes to run in, in priority order:

    1. Explicit LLM_MODE env var ("offline" | "ollama" | "cloud") always wins.
    2. Otherwise: no model env vars set at all -> "offline" (genuinely free,
       no network, no account -- see agents/offline.py).
    3. Otherwise: LLM_MODEL starts with "ollama/" -> "ollama" (free, local,
       runs against http://localhost:11434 by default, no API key needed).
    4. Otherwise -> "cloud" (any OpenAI-compatible endpoint: Groq, OpenAI,
       OpenRouter, etc. -- needs LLM_API_KEY).
    """
    explicit = os.getenv("LLM_MODE")
    if explicit:
        return explicit.lower()

    model = _env("LLM_MODEL", "OPENAI_MODEL")
    key = _env("LLM_API_KEY", "OPENAI_API_KEY")
    base = _env("LLM_BASE_URL", "OPENAI_API_BASE")

    if not model and not key and not base:
        return "offline"
...
def _build_llm(mode: str) -> LLM:
    """Builds the LLM for "ollama" or "cloud" mode. Never called in "offline" mode."""
    if mode == "ollama":
        # litellm needs the "ollama/<model>" prefix, not a bare model name +
        # base_url (verified: a bare name raises "LLM Provider NOT provided").
        # CrewAI's LLM class auto-defaults base_url to localhost:11434/v1 and
        # fills a dummy api_key, so neither is required for the local case --
        # set LLM_BASE_URL only if Ollama runs on a non-default host/port.
        model = _env("LLM_MODEL", default="ollama/llama3.2:3b")
        return LLM(
            model=model,
            base_url=_env("LLM_BASE_URL", "OPENAI_API_BASE"),  # None is fine -- CrewAI defaults it
            api_key=_env("LLM_API_KEY", "OPENAI_API_KEY", default="ollama"),
        )

    # cloud mode -- any OpenAI-compatible endpoint (Groq by default, but
    # also plain OpenAI, OpenRouter, etc. if LLM_BASE_URL is overridden).
    return LLM(
        model=_env("LLM_MODEL", "OPENAI_MODEL", default="groq/llama-3.1-70b-versatile"),
        api_key=_env("LLM_API_KEY", "OPENAI_API_KEY"),
        base_url=_env("LLM_BASE_URL", "OPENAI_API_BASE", default="https://api.groq.com/openai/v1"),
    )
...
class CrewAIAgent(AgentSystem):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.mode = _resolve_llm_mode()
        self.llm = _build_llm(self.mode) if self.mode != "offline" else None
